<template>
  ...
  <b-container fluid>
    ...
    <b-button to="nuevosingresos"> <b-icon icon="plus-circle"></b-icon>Nueva</b-button>
  ...
  export default {
    ...
    data() {
      return {
        ...
        accion:'',
        fields:[
          { key: 'numero', label: 'Ingreso', sortable: true, class: 'text-center' },

          { key: 'fecha', label: 'Fecha', sortable: true, class: 'text-center' },
          { key: 'observacion', label: 'Observacion', sortable: true, class: 'text-center' },

          { key: 'cant_prods', label: 'Cantidad de productos', sortable: true, class: 'text-center' },
    ...
    methods:{
      ...
      async getingresos(){
          const consulta=await APIProductos.getIngresos();
          this.datos=consulta
